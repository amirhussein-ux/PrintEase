...
const DashboardHeader: React.FC<DashboardHeaderProps> = ({ role, isDarkMode }) => {
	...
	const iconButtonBaseClasses =
		"inline-flex items-center justify-center p-3 rounded-lg transition-all duration-300 ease-out focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2";
	...
	return (
		<header className={`sticky top-0 z-[200] flex h-16 items-center justify-between border-b px-4 sm:px-6 relative ${headerThemeClasses}`}>
			...
			<div className="flex items-center gap-4 relative">
				<div ref={notifRef} className="relative z-[210]">
					<button
						title="Notifications"
						className={`inline-flex items-center justify-center p-3 rounded-lg transform focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 ${interactiveThemeClasses} ${iconColorClasses} cursor-pointer relative group`}
						onClick={toggleNotifications}
					>
						<BellIcon className={`h-6 w-6 group ${iconColorClasses}`} />
						{unreadCount > 0 && (
					...
					>
						<div className="absolute right-0 mt-3 w-96 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl shadow-xl z-[220] py-3 max-h-96 overflow-y-auto">
							...
							{notifications.length === 0 ? (
								<div className="px-4 py-8 text-center">
									<BellIcon className="h-12 w-12 text-gray-300 dark:text-gray-600 mx-auto mb-2" />
									<p className="text-sm text-gray-600 dark:text-gray-400">No notifications yet</p>
								</div>
							) : (
								<>
									<div className="flex justify-between items-center px-4 py-3 bg-gray-50/50 dark:bg-gray-700/50 mb-1">
										<button
											className="text-blue-600 dark:text-blue-400 text-sm font-medium hover:text-blue-700 dark:hover:text-blue-300 transition-all duration-300 ease-out transform hover:scale-105"
											onClick={async () => {
												if (!token) return;
												try {
													await axios.put(
										...
										<button
											className="text-red-600 dark:text-red-400 text-sm font-medium hover:text-red-700 dark:hover:text-red-300 transition-all duration-300 ease-out transform hover:scale-105 flex items-center gap-1"
											onClick={async () => {
												if (!token) return;
												try {
									...
									<div className="space-y-2 px-2 pt-2">
										{notifications.map((n) => (
											<div
												key={n._id}
												className={`group flex items-start justify-between p-4 rounded-xl transition-all duration-300 ease-out cursor-pointer ${
													n.read
														? "text-gray-700 dark:text-gray-300 hover:bg-gray-100/80 dark:hover:bg-gray-700/50"
														: "font-semibold text-gray-900 dark:text-white bg-blue-50/80 dark:bg-blue-900/50 hover:bg-blue-100/80 dark:hover:bg-blue-800/50"
											...
											>
												<div className="flex-1 min-w-0">
													...
													{n.description && <p className="text-gray-600 dark:text-gray-400 text-xs mt-1 line-clamp-2">{n.description}</p>}
													<p className="text-gray-400 dark:text-gray-500 text-xs mt-2">{new Date(n.createdAt).toLocaleDateString()}</p>
												</div>
												...
												<button
													...
													className="ml-2 p-2 rounded-lg text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-500/10 opacity-0 group-hover:opacity-100 transition-all duration-300 ease-out transform hover:scale-110"
