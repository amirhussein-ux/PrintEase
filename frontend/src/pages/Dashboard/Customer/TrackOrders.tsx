...
function statusBadgeClasses(status: OrderStatus) {
  switch (status) {
    case 'pending':
      return 'border-gray-400 text-gray-700 dark:text-gray-200 bg-gray-100 dark:bg-gray-400/10';
    case 'processing':
      return 'border-amber-400 text-amber-700 dark:text-amber-200 bg-amber-100 dark:bg-amber-400/10';
    case 'ready':
      return 'border-indigo-400 text-indigo-700 dark:text-indigo-200 bg-indigo-100 dark:bg-indigo-400/10';
    case 'completed':
      return 'border-green-400 text-green-700 dark:text-green-200 bg-green-100 dark:bg-green-400/10';
    case 'cancelled':
      return 'border-red-400 text-red-700 dark:text-red-200 bg-red-100 dark:bg-red-400/10';
    default:
      return 'border-gray-400 text-gray-700 dark:text-white/80 bg-gray-50 dark:bg-gray-700/50';
  }
}
  ...
  return (
    <DashboardLayout role="customer">
      <div className={`${BACKGROUND_GRADIENT} min-h-screen`}>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          ...
          <div className="mb-8 text-center">
            ...
            <p className={`${MUTED_TEXT} mt-2 text-sm md:text-base`}>Monitor your printing orders and their progress in real-time</p>
          </div>

          {/* Filters */}
          <div className="mb-8">
            <div className={`inline-flex flex-wrap gap-3 ${CARD_BACKGROUND} backdrop-blur-sm border border-gray-300 dark:border-gray-700 rounded-2xl p-3 shadow-lg`}>
                ...
                return (
                  <button
                    ...
                    className={`inline-flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-medium transition-all duration-200 border-2 ${
                      active
                        ? `${BUTTON_FILTER_ACTIVE} shadow-lg shadow-blue-500/25`
                        : `${BUTTON_FILTER_INACTIVE} hover:shadow-md`
                    }`}
                  >
                    <span>{label}</span>
                    <span className={`text-xs px-2 py-1 rounded-full border ${
                      active
                        ? 'border-blue-300 dark:border-white/40 bg-blue-200 dark:bg-white/20 text-blue-800 dark:text-white'
                        : 'border-gray-300 dark:border-gray-500 bg-gray-100 dark:bg-gray-700/50 text-gray-700 dark:text-gray-300'
                    }`}>
                      {counts[value] ?? 0}
                    </span>
                  </button>
          ...
          {loading && (
            <div className="flex justify-center mb-6">
              <div className={`${STATS_CARD} rounded-2xl p-6 text-center max-w-md w-full`}>
                <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-blue-500 mx-auto mb-3"></div>
                <div className={`${MUTED_TEXT} text-sm`}>Loading your orders...</div>
              </div>
            </div>
          )}
          {error && (
            <div className={`mb-6 rounded-2xl border border-red-500/40 bg-red-500/10 backdrop-blur-sm p-4 text-red-600 dark:text-red-300 text-sm flex items-center gap-3`}>
          ...
          <div className="grid grid-cols-1 gap-6">
            {!loading && filtered.length === 0 && (
              <div className={`rounded-2xl ${CARD_BORDER} bg-gradient-to-br ${IMAGE_BACKGROUND} p-12 text-center`}>
                ...
                <p className={`${MUTED_TEXT_LIGHT} text-sm`}>No orders match your current filter selection.</p>
              </div>
            )}
            ...
            {filtered.map((o) => {
              ...
              return (
                <div key={o._id} className={`rounded-2xl ${CARD_BORDER} bg-gradient-to-br ${IMAGE_BACKGROUND} p-6 shadow-xl hover:shadow-2xl transition-all duration-300 ${CARD_HOVER}`}>
                  <div className="flex flex-col lg:flex-row lg:items-start gap-6">
                    ...
                    <div className="flex-1 min-w-0 space-y-4">
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-center gap-3">
                        <div className="flex items-center gap-3">
                          <div className="text-gray-900 dark:text-white font-bold text-lg tracking-wide">{shortId(o._id)}</div>
                        ...
                        <div className={`text-xs ${MUTED_TEXT_LIGHT} font-mono tabular-nums bg-gray-100 dark:bg-gray-900/50 px-3 py-1.5 rounded-lg border border-gray-300 dark:border-gray-700`}>
                      ...
                      {/* Time Estimate */}
                      {getTimeRemaining(o) && (
                        <div className="flex items-center gap-2 text-sm text-blue-600 dark:text-blue-300 font-medium bg-blue-100 dark:bg-blue-500/10 border border-blue-300 dark:border-blue-500/20 px-3 py-2 rounded-lg">
                      ...
                      <div className="space-y-3">
                        <div className={`${SOFT_PANEL} p-4`}>
                          <div className="flex items-center gap-3 mb-2">
                            ...
                            <h4 className="text-gray-900 dark:text-white font-semibold text-sm">{first?.serviceName || 'Print Service'}</h4>
                          </div>
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs text-gray-600 dark:text-gray-300">
                            <div className="flex items-center gap-2">
                              <span className={`${MUTED_TEXT_LIGHT}`}>Quantity:</span>
                              <span className="font-medium">{first?.quantity} {first?.unit ? `· ${first.unit}` : ''}</span>
                            </div>
                            <div className="flex items-center gap-2">
                              <span className={`${MUTED_TEXT_LIGHT}`}>Options:</span>
                              <span className="font-medium truncate">{itemSummary(first)}</span>
                            </div>
                          </div>
                          {o.notes && (
                            <div className="mt-3 p-3 bg-gray-50 dark:bg-gray-800/50 rounded-lg border border-gray-300 dark:border-gray-700">
                              <div className="flex items-start gap-2 text-xs">
                                ...
                                <div>
                                  <span className={`${MUTED_TEXT} font-medium`}>Notes:</span>
                                  <span className="text-gray-700 dark:text-gray-300 ml-1">{o.notes}</span>
                                </div>
                              </div>
                            </div>
                          )}
                    ...
                    <div className="lg:w-64 space-y-4">
                      ...
                      <div className="text-right">
                        ...
                        <div className={`text-xs ${MUTED_TEXT_LIGHT} font-medium`}>Total Amount</div>
                      </div>
                          ...
                          <div className="space-y-2 max-h-32 overflow-y-auto">
                            {o.files.slice(0, 3).map((f) => (
                              <a
                                ...
                                className="flex items-center gap-2 text-xs text-blue-600 dark:text-blue-300 hover:text-blue-700 dark:hover:text-blue-200 transition-colors group"
                            ...
                            {o.files.length > 3 && (
                              <div className={`text-xs ${MUTED_TEXT_LIGHT} text-center pt-1 border-t border-gray-300 dark:border-gray-700`}>
                    ...
                    <div className="mt-6 p-6 rounded-xl border border-indigo-500/30 bg-gradient-to-r from-indigo-500/10 to-purple-500/10">
                      {!openQR[o._id] ? (
                        ...
                        <div className="flex flex-col items-center gap-4">
                          <div className="text-sm text-indigo-700 dark:text-indigo-200 font-medium">Show this QR code at pickup:</div>
                          ...
                          <div className="flex items-center gap-3">
                            <button
                              ...
                              className={`px-4 py-2 rounded-lg text-sm font-medium border border-gray-300 ${BUTTON_SECONDARY} hover:shadow-md transition-all duration-200 flex items-center gap-2`}
                            >
                              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <button
                              ...
                              className="px-4 py-2 rounded-lg text-sm font-medium border border-gray-300 dark:border-gray-600 bg-transparent text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-white/10 hover:text-gray-900 dark:hover:text-white transition-all duration-200"
                            >
                              Hide
                            </button>
                          </div>
      ...
      {showCancelModal && cancelTargetId && (() => {
        ...
        return (
          <div className="fixed inset-0 z-60 bg-black/50 dark:bg-black/70 backdrop-blur-sm flex items-center justify-center p-4" role="dialog" aria-modal="true" onClick={() => setShowCancelModal(false)}>
            <div className={`relative ${PANEL_SURFACE} shadow-2xl p-6 w-full max-w-sm`} onClick={(e) => e.stopPropagation()}>
              <div className="flex items-start gap-4">
                <div className="w-12 h-12 rounded-full bg-red-100 dark:bg-red-600/20 flex items-center justify-center">
                ...
                <div className="flex-1">
                  ...
                  <p className={`${MUTED_TEXT} text-sm mt-2`}>Are you sure you want to cancel this order {ord ? <span className="font-mono text-blue-600 dark:text-blue-300">{shortId(ord._id)}</span> : ''}? This action cannot be undone.</p>
                </div>
              </div>
              <div className="mt-6 flex items-center gap-3">
                <button onClick={() => { setShowCancelModal(false); setCancelTargetId(null); }} className={`flex-1 px-4 py-2 rounded-xl ${BUTTON_SECONDARY} transition-all`}>Keep Order</button>
                <button onClick={() => void confirmCancel()} disabled={updatingId === cancelTargetId} className={`flex-1 px-4 py-2 rounded-xl text-white font-semibold ${updatingId === cancelTargetId ? 'bg-gray-500 cursor-not-allowed' : 'bg-red-600 hover:bg-red-500'}`}>
      ...
      {enlargeQrFor && (
        ...
        >
          <div
            className={`relative ${PANEL_SURFACE} shadow-2xl p-6 w-[92vw] max-w-md`}
            onClick={(e) => e.stopPropagation()}
          >
            <button
              className="absolute top-4 right-4 bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-gray-600 px-3 py-1.5 text-sm font-bold rounded-lg text-gray-700 dark:text-white cursor-pointer transition-all duration-200 hover:shadow-md"
              onClick={() => setEnlargeQrFor(null)}
              aria-label="Close"
            >
              ✕
      ...
      {showReceiptFor && (() => {
        ...
        return (
          <div className={`fixed inset-0 z-50 ${MODAL_OVERLAY} backdrop-blur-sm flex items-center justify-center p-4`} role="dialog" aria-modal>
            <div className={`relative ${PANEL_SURFACE} shadow-2xl p-6 w-[92vw] max-w-md`}>
              <button
                className="absolute top-4 right-4 bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-gray-600 px-3 py-1.5 text-sm font-bold rounded-lg text-gray-700 dark:text-white cursor-pointer transition-all duration-200 hover:shadow-md"
              ...
              <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-2">Payment Receipt</h3>
              <p className={`${MUTED_TEXT} text-sm mb-6`}>Order <span className="font-mono text-blue-600 dark:text-blue-300">{shortId(ord._id)}</span> · {ord.receiptIssuedAt ? new Date(ord.receiptIssuedAt).toLocaleString() : ''}</p>
              
              <div className="space-y-4 text-sm">
                <div className="flex justify-between items-center py-2 border-b border-gray-300 dark:border-gray-700">
                  <span className={`${MUTED_TEXT}`}>Service</span>
                  <span className="font-medium text-gray-900 dark:text-white truncate max-w-[55%]">{first?.serviceName || 'Service'}</span>
                </div>
                <div className="flex justify-between items-center py-2 border-b border-gray-300 dark:border-gray-700">
                  <span className={`${MUTED_TEXT}`}>Quantity</span>
                  <span className="font-medium text-gray-900 dark:text-white">{first?.quantity}</span>
                </div>
                <div className="flex justify-between items-center py-2 border-b border-gray-300 dark:border-gray-700">
                  <span className={`${MUTED_TEXT}`}>Subtotal</span>
                  <span className="font-bold text-emerald-600 dark:text-emerald-400">{money(ord.subtotal, ord.currency || 'PHP')}</span>
                </div>
                <div className="flex justify-between items-center py-2 border-b border-gray-300 dark:border-gray-700">
                  <span className={`${MUTED_TEXT}`}>Paid</span>
                  <span className="font-medium text-gray-900 dark:text-white">{money(ord.paymentAmount || ord.subtotal, ord.currency || 'PHP')}</span>
                </div>
                <div className="flex justify-between items-center py-2 border-b border-gray-300 dark:border-gray-700">
                  <span className={`${MUTED_TEXT}`}>Change</span>
                  <span className="font-medium text-gray-900 dark:text-white">{money(ord.changeGiven || 0, ord.currency || 'PHP')}</span>
                </div>
                <div className="flex justify-between items-center py-2">
                  <span className={`${MUTED_TEXT}`}>Method</span>
                  <span className="font-bold text-blue-600 dark:text-blue-400 uppercase">{ord.paymentMethod || 'cash'}</span>
                </div>
              </div>
              ...
              <div className="mt-8 flex justify-end gap-3">
                <button
                  onClick={() => setShowReceiptFor(null)}
                  className={`px-5 py-2.5 rounded-xl ${BUTTON_SECONDARY} transition-all duration-200`}
                >
                  Close
                </button>
                <button
