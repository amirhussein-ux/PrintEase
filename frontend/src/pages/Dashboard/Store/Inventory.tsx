                            ...
                            <div className={`${PANEL_SURFACE} p-6`}>
                                ...
                                <ResponsiveContainer width="100%" height={300}>
                                    <PieChart>
                                        ...
                                        <Tooltip contentStyle={{ borderRadius: '12px', border: isDarkMode ? '1px solid #374151' : 'none', backgroundColor: isDarkMode ? '#1f2937' : '#ffffff' }} itemStyle={{ color: isDarkMode ? '#f3f4f6' : '#374151' }} />
                                    </PieChart>
                                </ResponsiveContainer>
                            </div>
                            <div className={`${PANEL_SURFACE} p-6`}>
                        ...
                        {activeTab === "products" && <div className="space-y-6">
                            ...
                            <div className="overflow-x-auto"><table className="w-full">
                                ...
                                <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                                    {(showArchivedProducts ? archivedProducts : filteredProducts).map((p: any) => <tr key={p._id} className={`${showArchivedProducts ? "bg-amber-50/50 dark:bg-amber-900/10" : ""} hover:bg-gray-50 dark:hover:bg-gray-700/30`}><td className="px-6 py-4 font-mono text-xs">{p.originalId ? p.originalId.slice(-6) : p._id.slice(-6)}</td><td className={showArchivedProducts ? "line-through" : ""}>{p.name}</td><td>{p.category ? <span className="px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 dark:bg-blue-900/30 text-blue-800 dark:text-blue-300">{p.category}</span> : '-'}</td><td className="text-center"><span className={`px-3 py-1 rounded-full text-sm font-medium ${p.amount <= p.minAmount ? 'bg-red-100 dark:bg-red-900/30 text-red-800 dark:text-red-300' : 'bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-300'}`}>{p.amount}</span></td><td className="text-center">{p.minAmount}</td><td className="text-right">₱{p.price.toFixed(2)}</td><td className="text-right">₱{p.entryPrice.toFixed(2)}</td><td className="px-6 py-4 text-right">{showArchivedProducts ? <button onClick={() => restoreArchivedProduct(p._id)} className="px-3 py-1 bg-green-600 text-white rounded text-xs">Restore</button> : <div className="flex justify-end gap-2"><button onClick={() => handleEdit(p._id)} className="p-2 text-blue-600 dark:text-blue-400"><PencilSquareIcon className="w-4 h-4" /></button><button onClick={() => setArchiveProductId(p._id)} className="p-2 text-amber-600 dark:text-amber-400"><ArchiveBoxIcon className="w-4 h-4" /></button></div>}</td></tr>)}
                                </tbody>
                            </table></div>
                        </div>}
